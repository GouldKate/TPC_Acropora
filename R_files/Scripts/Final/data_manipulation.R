...
setwd("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora")
getwd()

path.p<-"C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/Presens Measurements/All" #the location of all your respirometry files


#Change all Excel files to CSV
...
path.p<-"C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/Presens Measurements/All/csv" #the location of all your respirometry files
...
#Load Sample Info 
library(readxl)
Sample.Info <- read_excel("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/Raw_data/Sample.Info.xlsx")#read in sample.info data
View(Sample.Info)

# load surface area data
SA <- read_excel("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/Raw_data/SA.xlsx") #read sample.info data
View(SA)
...
for(i in 1:length(file.names.full)) { # for every file in list calculate O2 uptake or release rate and add the data to the Photo.R dataframe
  ...
  pdf(paste0("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora",rename,"thinning.pdf")) # open the graphics device
...
#Photo.R<-read.csv("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.R.csv") 
#Photo.R$X<-NULL
...
write.csv(Photo.R,"C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.R.csv") 
...
write.csv(Photo.R,"C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.Rwsampleinfo.csv") 
...
photo.blnk <- aggregate(umol.sec ~ fragment.ID.full*genotype*temp.Cat*light_dark*run*BLANK, data=Photo.R, mean)
View(photo.blnk)
...
colnames(photo.blnk)[5]<-'blank.rate' # rename the blank rate 

write.csv(photo.blnk, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/photo.blnk.all.csv") 
...
## average the Blnk1 and Blnk2 by run
## 
## ### checking data for any weirdness in blank rates by run and average of light and dark runs--- 30C is VERY low in many species on umol.cm2.s-1


df<-photo.blnk %>%
  group_by(run, temp.Cat) %>%
  summarise(Avg.blnk.rate=(mean(blank.rate)))
View(df)

df2<-photo.blnk %>%
  group_by(light_dark,temp.Cat) %>%
  summarise(Avg.blnk.rate=(mean(blank.rate)))
...
Photo.Rtest<-left_join(Photo.R,df)
View(Photo.Rtest)

# subtract the blanks######################
Photo.Rtest$umol.sec.corr<-Photo.Rtest$umol.sec-Photo.Rtest$Avg.blnk.rate
...
Photo.Rtest$umol.cm2.hr <- (Photo.Rtest$umol.sec.corr*3600)/Photo.Rtest$surf.area.cm2 #mmol cm-2 hr-1
Photo.Rtest<-Photo.Rtest[complete.cases(Photo.Rtest),] # # remove NAs and blanks
Photo.Rtest<-Photo.Rtest[Photo.Rtest$BLANK==0,]
Photo.Rtest <- na.omit(Photo.Rtest)

Photo.Rfinal<-Photo.Rtest
write.csv(Photo.Rfinal, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.Rfinal.csv") 
...
raw4<-ggplot(photo.Rrrr, aes(x=Temp.C, y=umol.cm2.hr, group = light_dark, col = fragment.ID))+
  geom_point()+  
  facet_wrap(~ light_dark*fragment.ID, labeller = labeller(.multi_line = FALSE))+
  theme(legend.position = "none")

esquisse:::ggplot_to_ppt("raw4")

raw5<-ggplot(photo.Rrrr, aes(x=Temp.C, y=umol.cm2.hr, group = light_dark, col = fragment.ID))+
  geom_point()+  
  facet_wrap(~ light_dark, labeller = labeller(.multi_line = FALSE))

esquisse:::ggplot_to_ppt("raw5")


write.csv(photo.Rrrr, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.RrespposiFINAL.csv") 


#Restart and look at FINAL data frame and calculate GROSS PHOTOSYNTHESIS
#
Photo.R2<-read.csv("C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.Rfinal.csv") 
...
#rename fragment ID 
Photo.R2$frag.ID<-Photo.R2$fragment.ID.full

write.csv(Photo.R2, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/Photo.R2NP_R.csv") 
...
write.csv(pgoss, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/np_R_GP.csv")
...
#### combined Pgross_Pnet-Rdark.csv, ad np_R_GP.csv
...
#write.csv(AllMeansG, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/June 2019/TPC_Output/AllMeansGfin.csv") 
write.csv(AllMeansG, "C:/Users/Kate/OneDrive - University of North Carolina at Chapel Hill/2019laptop/Documents/TPC_Acropora/R_files/R_Output/AllMeansGfin.csv") 
...
GPtb<-ggplot(subset(resp.datafin2,rate.type=="GP"), aes(x=genotype, umol.cm2.hr, colour=treatment, shape=genotype))+
  stat_summary(fun.data = "mean_cl_boot", colour = "black", size = 2)+
  geom_point(size=2)+
  #geom_errorbar(AllMeansT,aes(ymax=mean+se, ymin=mean-se), size=1.2, width=0.4) +
  ylab("GP(umol.cm2.hr)")+
  labs(title="Mean GP across genotype")+
  coord_flip()
GPtb
esquisse:::ggplot_to_ppt("GPtb")
#### Graph Pgross and Resp mean by genotype 
#### 



GPplotgeno3<-ggplot(AllMeansT, aes(x=genotype, y=Pgross.mean, color=genotype))+
  geom_point()

esquisse:::ggplot_to_ppt("GPplotgeno3")

Rplotgeno3<-ggplot(AllMeansT, aes(x=genotype, y=Rdark.mean, color=genotype))+
  geom_point()

esquisse:::ggplot_to_ppt("Rplotgeno3")

Npplotgeno3<-ggplot(AllMeansT, aes(x=genotype, y=Pnet.mean, color=genotype))+
  geom_point()

esquisse:::ggplot_to_ppt("Npplotgeno3")


AllMeansgenob<-AllMeansgeno%>%
  mutate(geno=substr(genotype, 1, nchar(genotype)-1))


GPplotgeno4<-ggplot(AllMeansG, aes(x=genotype, y=Pgross.mean, color=genotype))+
  geom_point()+
  facet_wrap(~genotype)

esquisse:::ggplot_to_ppt("GPplotgeno4")

Rplotgeno4<-ggplot(AllMeansG, aes(x=genotype, y=Rdark.mean, color=genotype))+
  geom_point()+
  facet_wrap(~genotype)

esquisse:::ggplot_to_ppt("Rplotgeno4")

NPplotgeno4<-ggplot(AllMeansG, aes(x=genotype, y=Pnet.mean, color=genotype))+
  geom_point()+
  facet_wrap(~genotype)

esquisse:::ggplot_to_ppt("NPplotgeno4")
