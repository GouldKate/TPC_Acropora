...
Fig1<-T1plot+T2TPC+T3TPC+CTPC
ggexport(Fig1, filename = "Fig1.pdf")
...
# run bootstrap over many curves ####
boot_manyT <- group_by(mydata_M50Cd, treatment, rate.type) %>%
  # create 200 bootstrap replicates per curve
  do(., boot = modelr::bootstrap(., n = 20, id = 'boot_num')) %>%
  # unnest to show bootstrap number, .id
  unnest() %>%
  # regroup to include the boot_num
  group_by(., treatment, rate.type, boot_num) %>%
  # run the model using map()
  mutate(fit = map(strap, ~nls_multstart(log.rate ~ schoolfield_high(lnc, E, Eh, Th, temp = K, Tc = 20),
                        data = data.frame(.),
                        iter = 50,
...
preds_manyT <- boot_manyT %>%
  ...
  dplyr::summarise(lwr_CI = quantile(.fitted, 0.025),
            upr_CI = quantile(.fitted, 0.975)) %>%
  ungroup() %>%
  merge(., preds_manyT, by = c("K", "treatment", "rate.type")) %>%
  # merge with max_min to delete predictions outside of the max and min temperatures of each curve
...
#preds_manyTreat<-read.csv("C:/Github/TPC_Acropora/R_output_Data_Frames/preds_manyTreat.csv")

treat1PTreat<-preds_manyTreat%>%
  filter(treatment=="T1")
treat2PTreat<-preds_manyTreat%>%
  filter(treatment=="T2")
treat3PTreat<-preds_manyTreat%>%
  filter(treatment=="T3")
treatCPTtreat<-preds_manyTreat%>%
  filter(treatment=="C")
```
...
T1Tplot<-ggplot(treat1, aes(K - 273.15, log.rate,shape=rate.type)) +
  ...
  geom_smooth(data=treat1PT, aes(x=(K - 273.15), y=log.rate),size=1.25) +
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab('log Metabolic rate') +
  xlab('Temperature (ºC)') +
  facet_wrap(~ rate.type, labeller = labeller(.multi_line = FALSE)) +
...
T1Tplotreat<-ggplot(treat1, aes(K - 273.15, log.rate,shape=rate.type)) +
  geom_point(size=2) +
  geom_ribbon(aes(ymin = lwr_CI, ymax = upr_CI), data = treat1PTreat, alpha = .35) + 
  geom_smooth(data=treat1PTreat, aes(x=(K - 273.15), y=log.rate),size=1.25, colour="#66CCFF") +
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab('log Metabolic rate') +
  xlab('Temperature (ºC)') +
  facet_wrap(~ rate.type, labeller = labeller(.multi_line = FALSE)) +
...
T1Tplotreat
ggexport(T1Tplotreat, filename = "Fig2acolor.pdf")
...
T2Tplotreat<-ggplot(treat2, aes(K - 273.15, log.rate, shape=rate.type)) +
  geom_point(size=2) +
  geom_ribbon(aes(ymin = lwr_CI, ymax = upr_CI), data = treat2PTreat, alpha = .35) + 
  geom_smooth(data=treat2PTreat, aes(x=(K - 273.15), y=log.rate),size=1.25, colour="#FFFF00") +
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab('log Metabolic rate') +
  xlab('Temperature (ºC)') +
  facet_wrap(~ rate.type, labeller = labeller(.multi_line = FALSE)) +
...
ggexport(T2Tplotreat, filename = "Fig2bcolor.pdf")
...
T3Tplotreat<-ggplot(treat3, aes(K - 273.15, log.rate, shape=rate.type)) +
  geom_point(size=2) +
  geom_ribbon(aes(ymin = lwr_CI, ymax = upr_CI), data = treat3PTreat, alpha = .35) + 
  geom_smooth(data=treat3PTreat, aes(x=(K - 273.15), y=log.rate),size=1.25, colour="#FF00CC") +
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab('log Metabolic rate') +
  xlab('Temperature (ºC)') +
  facet_wrap(~ rate.type, labeller = labeller(.multi_line = FALSE)) +
...
ggexport(T3Tplotreat, filename = "Fig2ccolor.pdf")
```
'#66CCFF','#FFFF00', '#FF00CC', '#66FF33'
...
TCTplotreat<-ggplot(treatC, aes(K - 273.15, log.rate, shape=rate.type)) +
  geom_point(size=2) +
  geom_ribbon(aes(ymin = lwr_CI, ymax = upr_CI), data = treatCPTtreat, alpha = .35) + 
  geom_smooth(data=treatCPTtreat, aes(x=(K - 273.15), y=log.rate),size=1.25, colour="#66FF33") +
  theme_bw(base_size = 12, base_family = 'Helvetica') +
  ylab('log Metabolic rate') +
  xlab('Temperature (ºC)') +
  facet_wrap(~ rate.type, labeller = labeller(.multi_line = FALSE)) +
...
ggexport(TCTplotreat, filename = "Fig2dcolor.pdf")


Fig2<-T1Tplotreat+T2Tplotreat+T3Tplotreat+TCTplotreat

ggexport(Fig2, filename = "Fig2.pdf")
...
PmaxGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Pmax GP values",
      col.names = c("Genotype", "Treatment", "n", "Pmax","SD", "SE", "CI"),
      align = 'c') %>%
...
SSt
PmaxgGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Pmax GP values",
      col.names = c("Genotype", "n", "Pmax","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
SSt
PmaxtGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Pmax GP values",
      col.names = c("Treatment", "n", "Pmax","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
SSt
RmaxGRsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Rmax R values",
      col.names = c("Genotype", "Treatment", "n", "Rmax","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
SSt
RmaxGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Rmax R values",
      col.names = c("Genotype", "n", "Rmax","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
RmaxGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Rmax R values",
      col.names = c("Treatment", "n", "Rmax","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
ToptGPsum<-kable(SSt, digits = 4,
      caption = "Summary statistics for Topt G{ values",
      col.names = c("Treatment", "n", "Topt","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
                full_width = F, position = "center")
...
```{r}
RmaxRsum<-parametersFragGRrmax %>% 
  group_by(genotype,treatment) %>%
  get_summary_stats(rmax, type = "common")


RmaxRsumT<-kable(RmaxRsum, digits = 4,
      caption = "Summary statistics for Rmax R values",
...
PmaxGPsum<-parametersFragGPPmax %>% 
  group_by(genotype,treatment) %>%
  get_summary_stats(rmax, type = "common")

PmaxGPsumT<-kable(RmaxRsum, digits = 4,
      caption = "Summary statistics for Pmax GP values",
      col.names = c("Treatment", "Genotype", "Parameter", "n", "Min", "Max", "Median", "IQR", "Mean","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
...
summary(PmaxImod)
## F- 2.87, DF- 35 p-value 0.008684**
r2(PmaxImod)
# R2: 0.474
#   adj. R2: 0.309
 #how well the model fits the data "goodness-of-fit"
...
# R2 for Linear Regression R-squared is a statistical measure of how close the data are to the fitted regression line
# #AKA the coefficient of determination, or the coefficient of multiple determination for multiple regression
...
check_model(PmaxImod)
model_performance(PmaxImod)
m1 <- lm(rmax~treatment,parametersFragGPPmax)
m2 <- lm(rmax~treatment+genotype,parametersFragGPPmax)
m3 <- lm(rmax~treatment*genotype,parametersFragGPPmax)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
...
PmaxgmodPTu<-ggboxplot(parametersFragGPPmax, x = "treatment", y = "rmax", fill="treatment")+
  stat_pvalue_manual(
    stat.testi, label = "p.adj", 
    y.position = c(.9,1,1.1, 1.2,1.3, 1.4))
plot2<-PmaxgmodPTu+
  scale_fill_manual(values = c('#66CCFF','#FFFF00', '#FF00CC', '#66FF33'))+
  scale_x_discrete(breaks=c("T1", "T2", "T3", "TC"),
        labels=c("Sprinkler", "Feeding", "Interactive","Control"))
ggexport(plot2, filename = "PmaxANOplottreaTUK.pdf")
```
...
```{r}
library(rstatix)
ToptGPsum<-parametersFragGPTopt %>% 
  group_by(genotype,treatment) %>%
  get_summary_stats(topt, type = "common")

ToptGPsumT<-kable(ToptGPsum, digits = 4,
      caption = "Summary statistics for Topt GP values",
      col.names = c("Treatment", "Genotype", "Parameter", "n", "Min", "Max", "Median", "IQR", "Mean","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
...
```{r}
ToptGRsum<-parametersFragGRTopt %>% 
  group_by(genotype,treatment) %>%
  get_summary_stats(topt, type = "common")

ToptGRsumT<-kable(ToptGRsum, digits = 4,
      caption = "Summary statistics for Topt R values",
      col.names = c("Treatment", "Genotype", "Parameter", "n", "Min", "Max", "Median", "IQR", "Mean","SD", "SE", "CI"),
      align = 'c') %>%
  kable_styling(bootstrap_options =
                  c("hover", "responsive", "striped"),
...
breadGRsum<-parametersFragGRbrea %>% 
  group_by(genotype) %>%
  get_summary_stats(breadth, type = "common")

breadGRsumT<-kable(breadGRsum, digits = 4,
      caption = "Summary statistics for Breadth R values",
...
#   R2: 0.257
#   adj. R2: 0.009
check_model(ctminImod)
model_performance(ctminImod)
m1 <- lm(ctmin~treatment,parametersFragRFc)
m2 <- lm(ctmin~genotype+treatment,parametersFragRFc)
m3 <- lm(ctmin~genotype*treatment,parametersFragRFc)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
...
#    adj. R2: 0.222
check_model(eImod)
model_performance(eImod)
m1 <- lm(e~treatment,parametersFragGRE)
m2 <- lm(e~genotype+treatment,parametersFragGRE)
m3 <- lm(e~genotype*treatment,parametersFragGRE)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
...
#      adj. R2: 0.096
check_model(ehrImod)
model_performance(ehrImod)
m1 <- lm(eh~treatment,parametersFragGREho)
m2 <- lm(eh~genotype+treatment,parametersFragGREho)
m3 <- lm(eh~genotype*treatment,parametersFragGREho)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
#m2
...
#      adj. R2: -0.093
check_model(ehpImod)
model_performance(ehpImod)
m1 <- lm(eh~treatment,parametersFragGPEh)
m2 <- lm(eh~genotype+treatment,parametersFragGPEh)
m3 <- lm(eh~genotype*treatment,parametersFragGPEh)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
#m1
...
#     R2: 0.171
#      adj. R2: -0.154
check_model(q10pImod)
model_performance(q10pImod)
m1 <- lm(q10~treatment,parametersFragGPq10)
m2 <- lm(q10~genotype+treatment,parametersFragGPq10)
m3 <- lm(q10~genotype*treatment,parametersFragGPq10)
compare_performance(m1,m2,m3) #choose a model that has a high  R2, low AIC and BIC low Root mean square
plot(compare_performance(m1, m2, m3))
test_performance(m1, m2, m3) #models are nested
#m1
